...
| --- | --- | --- |
| Expand style taxonomy from SPEC2 | Add Flat Design, Skeuomorphism, Claymorphism, Dark Futuristic, Web 2.0 Gloss / Frutiger Aero, and classify design-system languages versus patterns. | unsliced |
| Style-aware ResearchDossier | Implement `style.id -> dossierVariant` so the dossier changes density, framing, typography emphasis, and preview anatomy by style. Phase 1 is now tracked by `US-003` and `docs/design/phase-1-visual-dossier-plan.md`. | active |
